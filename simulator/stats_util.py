...
class StatsUtil:
    ...
    def dump(self, save_path, prefix=""):
        ...
        assert len(self._macro_comp_use) == len(self._macro_col_use) == len(self._macro_cell_total), f"{self._macro_comp_use=}, {self._macro_col_use=}, {self._macro_cell_total=}"
        macro_cell_use = [comp * col for comp, col in zip(self._macro_comp_use, self._macro_col_use)]
        sum_macro_cell_use = sum(macro_cell_use)
        sum_macro_cell_total = sum(self._macro_cell_total)
        sample_macro_comp_use = self._macro_comp_use# [:min(100,len(self._macro_comp_use))]
        sample_macro_col_use = self._macro_col_use# [:min(100,len(self._macro_col_use))]
        cell_ultilization_data = {
            "use_rate": sum_macro_cell_use / sum_macro_cell_total if sum_macro_cell_total>0 else 0,
