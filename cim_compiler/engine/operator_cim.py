...
class Operator:
    ...
    def compile(self, code_dir, image_kwargs={}):
        ...
        case_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), self.template_path
        )
...
class DenseConv2dOperator(Operator):
    ...
    def compile_and_run_from_dataflow_dir(self, df_dir, code_dir, check_result=False):
        # read data
        input = np.loadtxt(
            os.path.join(df_dir, "conv_input_feature.txt"), dtype=np.int8
        )
        ...
        weight = weight.reshape(
            self.op_config["out_channel"],
            self.op_config["in_channel"],
            self.op_config["ker_size"],
            self.op_config["ker_size"],
        )
        ...
        output = self.compile_and_run(
            code_dir, image_kwargs={"input": input, "weight": weight}
        )
...
class DenseLinearOperator(Operator):
    ...
    def compile_and_run_from_dataflow_dir(self, df_dir, code_dir, check_result=False):
        ...
        weight = np.loadtxt(os.path.join(df_dir, "weight.txt"), dtype=np.int8)
        bias = np.loadtxt(os.path.join(df_dir, "bias.txt"), dtype=np.int32)
        scale = np.loadtxt(os.path.join(df_dir, "scale.txt"), dtype=np.float32).reshape(
            -1
        )
        out_zp = np.loadtxt(
            os.path.join(df_dir, "qo.zero_point.txt"), dtype=np.int32
        ).reshape(1)
...
class DenseConv2dQuantifyOperator(Operator):
    ...
    def compile_and_run_from_dataflow_dir(self, df_dir, code_dir, check_result=False):
        ...
        golden_i32 = np.loadtxt(
            os.path.join(df_dir, "output_feature.txt"), dtype=np.int32
        )
        golden_i8 = np.loadtxt(os.path.join(df_dir, "output.txt"), dtype=np.int8)
        ...
        # compile and run, get output
        output_i8 = self.compile_and_run(
            code_dir,
            image_kwargs={
                "input": input,
                "weight": weight,
