...
class ModelRunner:
    def __init__(self, model_name, model_path, code_dir, is_bit_sparse=False, is_value_sparse=False):
    ...
    def find_first_valid_op_template(self, raw_layer):
        for op_template in OP_TEMPLATE_LIST:
            if op_template.check_raw_layer(raw_layer, self.is_value_sparse, self.is_bit_sparse):
                return op_template
        return None
