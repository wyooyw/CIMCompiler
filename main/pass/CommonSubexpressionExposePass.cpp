...
static std::pair<mlir::Value, mlir::Value> getAddIntegerOperators(Value operand){
    auto add_op = operand.getDefiningOp<arith::AddIOp>();
    if (!add_op){
...
static std::pair<mlir::Value, mlir::Value> getMulIntegerOperators(Value operand){
    auto mul_op = operand.getDefiningOp<arith::MulIOp>();
    if (!mul_op){
        std::cerr << "getAddInteger fail" << std::endl;
        std::exit(1);
...
namespace {
    ...
    struct AddiAddPattern : public OpRewritePattern<arith::AddIOp> {
      ...
      matchAndRewrite(arith::AddIOp op, PatternRewriter &rewriter) const final {
        ...
        if(isConstant(lhs) || isConstant(rhs)) return failure();
    ...
    struct MuliDiviPattern : public OpRewritePattern<arith::DivSIOp> {
      ...
      matchAndRewrite(arith::DivSIOp op, PatternRewriter &rewriter) const final {
        ...
        mlir::Value c3 = rewriter.create<arith::ConstantIndexOp>(op.getLoc(), mul_factor);
...
struct CommonSubexpressionExposePass
    : public PassWrapper<CommonSubexpressionExposePass, OperationPass<ModuleOp>> {
...
void CommonSubexpressionExposePass::runOnOperation() {
  ...
  patterns.add<AddiAddPattern, AddiMuliPattern, AddiAddiPattern, MuliDiviPattern>(
      &getContext());
      // ForOpConvert
  ...
  if (failed(
    ...
    signalPassFailure();
  std::cout << "CommonSubexpressionExposePass::runOnOperation finish!" << std::endl;
}
