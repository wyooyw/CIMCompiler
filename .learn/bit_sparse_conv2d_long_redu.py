...
from op.bit_sparse.bit_sparse_conv2d_strategy_base import (
    BitSparseConv2dConfig,
    BitSparseConv2dStrategyBase,
)
from op.bit_sparse.weight_transform import (
    csd_to_int,
...
from op.code_emitter import CodeEmitter, Tensor
...
class BitSparseConv2dLongReduceStrategy(BitSparseConv2dStrategyBase):
    ...
    def check_config(self, cim_cfg, op_cfg):
        ...
        checker.check(
            op_cfg.is_depthwise == False,
            f"BitSparseConv2dLongReduceStrategy not support dw-conv, but got op_cfg.is_depthwise={op_cfg.is_depthwise}",
        )

        # Align check
        checker.check(
            op_cfg.in_channel >= cim_cfg.n_compartment,
            f"Expect op_cfg.in_channel > cim_cfg.n_compartment, but got op_cfg.in_channel={op_cfg.in_channel}, cim_cfg.n_compartment={cim_cfg.n_compartment}",
        )
        checker.check(
            op_cfg.in_channel % cim_cfg.n_compartment == 0,
            f"Expect op_cfg.in_channel % cim_cfg.n_compartment==0, but got op_cfg.in_channel={op_cfg.in_channel} , cim_cfg.n_compartment={cim_cfg.n_compartment}",
        )
        ...
        checker.check(
            total_row > cim_cfg.n_row,
            f"Expect total_row > cim_cfg.n_row, but got total_row={total_row} cim_cfg.n_row={cim_cfg.n_row}",
        )
        checker.check(
            self.decide_tile_in_channel(cim_cfg, op_cfg) > 0,
            "cannot find a tile_in_channel",
        )
        ...
        checker.check(
            use_pim_buffer <= cim_cfg.pim_buffer_size,
            f"use_pim_buffer <= cim_cfg.pim_buffer_size, but got use_pim_buffer={use_pim_buffer}, cim_cfg.pim_buffer_size={cim_cfg.pim_buffer_size}",
        )
    ...
    def decide_tile_in_channel(self, cim_cfg, op_cfg):
        ...
        while True:
            use_row = (
                (tile_in_channel // cim_cfg.n_compartment)
                * op_cfg.ker_width
                * op_cfg.ker_height
            )
    ...
    def get_low_level_code(
        self, cim_cfg, op_cfg, param_cfg, input_on_local=True, output_on_local=True
    ):
        """
        ...
        # Alloc input
        if input_on_local:
            input_local_memory = Tensor(
                name=op_cfg.in_name,
                shape=[op_cfg.in_height, op_cfg.in_width, op_cfg.in_channel],
            )
        else:
            input_size = op_cfg.in_height * op_cfg.in_width * op_cfg.in_channel
            input_global_memory = Tensor(
                name=op_cfg.in_name,
                shape=[op_cfg.in_height, op_cfg.in_width, op_cfg.in_channel],
            )
            input_local_memory = emitter._emit_malloc(
                dtype=op_cfg.in_dtype,
                scope="local.mem",
                name=f"{op_cfg.in_name}_localmem",
                size=input_size,
                shape=[op_cfg.in_height, op_cfg.in_width, op_cfg.in_channel],
            )
            # Load input from global memory to local memory
            emitter._emit_trans(
                saddr_base=input_global_memory,
                saddr_offset=0,
                sscope="global",
                daddr_base=input_local_memory,
                daddr_offset=0,
                dscope="local.mem",
                size=input_size,
            )
        ...
        weight_global_memory = Tensor(
            name=f"{op_cfg.weight_name}", shape=op_cfg.weight_shape
        )
        weight_local_memory = emitter._emit_malloc(
            dtype="int8",
            scope="local.wmem",
            size=op_cfg.bytes_weight,
            name=f"{op_cfg.weight_name}",
            shape=[
                op_cfg.weight_shape[0],
                op_cfg.ker_height,
                op_cfg.ker_width,
                op_cfg.in_channel,
        ...
        emitter._emit_trans(
            saddr_base=weight_global_memory,
            saddr_offset=0,
            sscope="global",
            daddr_base=weight_local_memory,
            daddr_offset=0,
            dscope="local.wmem",
            size=op_cfg.bytes_weight,
        )
        ...
        info_global_memory = emitter._emit_malloc(
            dtype="int8",
            scope="global",
            name=f"{op_cfg.info_name}",
            size=op_cfg.bytes_info,
            shape=[
                op_cfg.bytes_info,
            ],
        )
        info_local_memory = emitter._emit_malloc(
            dtype="int8",
            scope="local.info",
            name=f"{op_cfg.info_name}_localmem",
            size=op_cfg.bytes_info,
            shape=[
                op_cfg.info_shape[0],
                ...
                op_cfg.ker_height,
                op_cfg.ker_width,
                tile_in_channel,  # info.shape[1]
                cim_cfg.n_macro,
                op_cfg.info_shape[2] // cim_cfg.n_macro,
            ],
        )
        emitter._emit_trans(
            saddr_base=info_global_memory,
            saddr_offset=0,
            sscope="global",
            daddr_base=info_local_memory,
            daddr_offset=0,
            dscope="local.info",
            size=op_cfg.bytes_info,
        )
        ...
        # Alloc output
        if output_on_local:
            output_local_memory = Tensor(
                name=f"{op_cfg.out_name}",
                shape=[op_cfg.out_height, op_cfg.out_width, op_cfg.out_channel],
            )
        else:
            output_size = op_cfg.out_channel * op_cfg.out_height * op_cfg.out_width
            output_global_memory = Tensor(
                name=f"{op_cfg.out_name}",
                shape=[op_cfg.out_height, op_cfg.out_width, op_cfg.out_channel],
            )
            output_local_memory = emitter._emit_malloc(
                dtype=op_cfg.out_dtype,
                scope="local.mem",
                name=f"{op_cfg.out_name}_localmem",
                size=output_size,
                shape=[op_cfg.out_height, op_cfg.out_width, op_cfg.out_channel],
            )

        # Output temp memory for sum
        output_temp_local_memory = emitter._emit_malloc(
            dtype=op_cfg.out_dtype,
            scope="local.mem",
            name=f"{op_cfg.out_name}_temp_localmem",
            size=op_cfg.out_channel,
            shape=[
                op_cfg.out_channel,
            ],
        )
        ...
        # Alloc input buffer
        input_pim_buffer = emitter._emit_malloc(
            dtype=op_cfg.in_dtype,
            scope="local.pbuf",
            name=f"{op_cfg.in_name}_pbuf",
            size=cim_cfg.n_compartment,
            shape=[
                cim_cfg.n_compartment,
            ],
        )

        # Alloc output buffer
        output_pim_buffer_size = cim_cfg.bits_column * cim_cfg.n_macro
        output_pim_buffer = emitter._emit_malloc(
            dtype=op_cfg.out_dtype,
            scope="local.pbuf",
            size=output_pim_buffer_size,
            name=f"{op_cfg.out_name}_pbuf",
        )
        ...
        weight_macro_size = (
            ...
            * cim_cfg.n_compartment
            * cim_cfg.n_macro
            * cim_cfg.bytes_column
        )
        weight_macro = emitter._emit_malloc(
            dtype="int8",
            scope="local.macro",
            name=f"{op_cfg.weight_name}_macro",
            size=weight_macro_size,
            shape=[
                cim_cfg.n_row,
                cim_cfg.n_compartment,
                cim_cfg.n_macro,
        ...
        for oc_time_step, oc in enumerate(fold):
            for o_ic in range(op_cfg.in_channel // tile_in_channel):
                ...
                n_element_in_one_filter = (
                    tile_in_channel * op_cfg.ker_height * op_cfg.ker_width
                )
                weight_macro_use_size = (
                    n_element_in_one_filter * cim_cfg.n_macro * cim_cfg.bytes_column
                )
                assert weight_macro_use_size <= weight_macro_size
                assert (
                    tile_in_channel % cim_cfg.n_compartment == 0
                ), f"tile_in_channel={tile_in_channel},cim_cfg.n_compartment={cim_cfg.n_compartment}"
                ...
                for hk in range(op_cfg.ker_height):
                    for wk in range(op_cfg.ker_width):
                        for i_ic in range(tile_in_channel):
                            total_row = (
                                hk * op_cfg.ker_width * tile_in_channel
                                + wk * tile_in_channel
                                + i_ic
                            )
                            ...
                            emitter._emit_pim_store(
                                saddr_base=weight_local_memory,
                                saddr_offset=weight_local_memory.get(
                                    oc_time_step, hk, wk, ic_idx, 0
                                ),
                ...
                pim_load_size = (
                    num_in_filter * cim_cfg.n_macro * cim_cfg.bits_column * 3 // 8
                )
                # pim_load_size = num_in_filter * use_macro * cim_cfg.bits_column * 3 // 8
                emitter._emit_pim_load_info(
                    addr_base=info_local_memory,
                    addr_offset=info_local_memory.get(
                        oc_time_step, o_ic, 0, 0, 0, 0, 0
                    ),
                    size=pim_load_size,
                )
                ...
                for ho in range(0, op_cfg.out_height):
                    for wo in range(0, op_cfg.out_width):
                        ...
                        # Clear output buffer
                        emitter._emit_pim_buffer_clear(
                            addr_base=output_pim_buffer,
                            addr_offset=0,
                            length=output_pim_buffer_size,
                        )
                        ...
                        for hk in range(0, op_cfg.ker_height):
                            for wk in range(0, op_cfg.ker_width):
                                for i_ic in range(
                                    0, tile_in_channel // cim_cfg.n_compartment
                                ):
                                    ...
                                    # Check padding
                                    if (
                                        pad_hi < op_cfg.padding[0]
                                        or pad_hi
                                        >= op_cfg.in_height + op_cfg.padding[0]
                                        or pad_wi < op_cfg.padding[1]
                                        or pad_wi >= op_cfg.in_width + op_cfg.padding[1]
                                    ):
                                        continue
                                    ...
                                    emitter._emit_ldbuf(
                                        saddr_base=input_local_memory,
                                        saddr_offset=input_local_memory.get(
                                            in_height_idx, in_width_idx, in_channel_idx
                                        ),
                                    ...
                                    emitter._emit_pim_mult_csd(
                                        iaddr_base=input_pim_buffer,
                                        iaddr_offset=0,
                        ...
                        if all([i == 2 for i in oc]):
                            ...
                            if o_ic == 0:
                                emitter._emit_stbuf(
                                    saddr_base=output_pim_buffer,
                                    saddr_offset=0,
                                    daddr_base=output_local_memory,
                                    daddr_offset=output_local_memory.get(
                                        ho, wo, out_channel_idx
                                    ),
                                    size=len(oc),
                                )
                            else:
                                emitter._emit_stbuf(
                                    saddr_base=output_pim_buffer,
                                    saddr_offset=0,
                                    daddr_base=output_temp_local_memory,
                                    daddr_offset=0,
                                    size=len(oc),
                                )
                                emitter._emit_veadd(
                                    saddr0_base=output_local_memory,
                                    saddr0_offset=output_local_memory.get(
                                        ho, wo, out_channel_idx
                                    ),
                                    saddr1_base=output_temp_local_memory,
                                    saddr1_offset=0,
                                    daddr_base=output_local_memory,
                                    daddr_offset=output_local_memory.get(
                                        ho, wo, out_channel_idx
                            ...
                            if o_ic == 0:
                                ...
                                # ipdb.set_trace()
                                if 0 in oc:
                                    emitter._emit_memset(
                                        addr_base=output_temp_local_memory,
                                        addr_offset=0,
                                ...
                                for i in range(len(out_mask)):
                                    ...
                                    if case1 or case2 or case3:
                                        out_channel_delta_index_filled = col_to_filter[
                                            i
                                        ]
                                        ...
                                        emitter._emit_stbuf(
                                            ...
                                            saddr_offset=i,
                                            daddr_base=output_temp_local_memory,
                                            daddr_offset=output_temp_local_memory.get(
                                                out_channel_delta_index_filled
                                            ),
                                            size=1,
                                        )
        ...
        if not output_on_local:
            ...
            emitter._emit_trans(
                saddr_base=output_local_memory,
                saddr_offset=0,
                sscope="local.mem",
                daddr_base=output_global_memory,
                daddr_offset=0,
                dscope="global",
                size=output_size,
            )
...
def run(weight, bsp_conv2d_cfg):
    ...
    code = strategy.get_low_level_code(
        cim_cfg, bsp_conv2d_cfg, param_cfg, new_weight, info, fold
    )
    print(code)
...
def test(weight, bsp_conv2d_cfg):
    ...
    new_weight = new_weight.reshape(
        new_weight.shape[0], new_weight.shape[1], cim_cfg.bits_column * cim_cfg.n_macro
    )
    info = info.reshape(
        new_weight.shape[0],
        new_weight.shape[1],
        cim_cfg.bits_column * cim_cfg.n_macro,
        3,
    )
...
if __name__ == "__main__":
    ...
    CimConfig.GLOBAL_CIM_CONFIG = CimConfig(
        n_macro=4,
        n_compartment=32,
        n_row=64,
        bytes_column=2,
        local_memory_size=local_memory_size,
        pim_buffer_size=pim_buffer_size,
        info_size=info_size,
    )

    bsp_conv2d_cfg = BitSparseConv2dConfig(
        in_name="I",
        weight_name="W",
        info_name="INFO",
        out_name="O",
    ...
    weight = generate_valid_weight(
        [
            bsp_conv2d_cfg.out_channel,
            bsp_conv2d_cfg.in_channel,
            bsp_conv2d_cfg.ker_height,
            bsp_conv2d_cfg.ker_width,
        ]
    )
    # print(weight)
    ...
    conv = torch.nn.Conv2d(
        out_channels=bsp_conv2d_cfg.out_channel,
    ...
    in_shape = (
        bsp_conv2d_cfg.in_channel * bsp_conv2d_cfg.in_height * bsp_conv2d_cfg.in_width
    )
    input = torch.Tensor(
        np.arange(1, in_shape + 1, dtype="int8").reshape(
            bsp_conv2d_cfg.in_channel, bsp_conv2d_cfg.in_height, bsp_conv2d_cfg.in_width
        )
    ).to(torch.float)
