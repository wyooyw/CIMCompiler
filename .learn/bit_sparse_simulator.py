...
from op.bit_sparse.weight_transform import csd_to_int, recover_csd
from simulator.simple_simulator import Macro, Memory, Simulator, SimulatorExecutor
...
class BitSparseSimulator(Simulator):
    ...
    def _run_setpw(self, code):
        ...
        assert self.macro_ibw in [
            8,
            16,
            32,
        ], f"Currently, we only support macro_ibw is divisable by byte, but we got macro_ibw={self.macro_ibw}"
        assert self.macro_wbw in [
            1,
            8,
            16,
            32,
        ], f"Currently, we only support macro_wbw is 1 in bit_sparse mode, but we got macro_wbw={self.macro_wbw}"
        assert self.macro_obw in [
            8,
            16,
            32,
        ], f"Currently, we only support macro_obw is divisable by byte, but we got macro_obw={self.macro_obw}"
    ...
    def _run_pim_mult_csd(self, code):
        ...
        assert (
            self.macro_ibw % 8 == 0
        ), f"Currently, we only support macro.ibw is align by byte, but we got macro_ibw={self.macro_ibw}"
        isize = isize_bit // 8
        ...
        for comp_idx in range(code["comp_off"]):
            waddr = (
                code["row_off"]
                * (
                    self.cim_cfg.n_compartment
                    * self.cim_cfg.n_macro
                    * self.cim_cfg.bytes_column
                )
            ...
            wdata += wdata_part
        wtensor = np.frombuffer(wdata, dtype=f"int8").reshape(
            code["comp_off"], self.cim_cfg.get_n_column(f"int8") * macro_num
        )
        # print("wtensor:",wtensor)
        wtensor = tensor_int8_to_bits(wtensor)
        wtensor = wtensor.reshape(
            code["comp_off"], self.cim_cfg.bits_column * macro_num
        )

        # Get info matrix
        info_begin = code["info_offset"]
        info_size = (
            code["comp_off"] * self.cim_cfg.n_macro * self.cim_cfg.bits_column * 3 // 8
        )
        ...
        info_tensor = info_tensor.reshape(
            code["comp_off"], self.cim_cfg.n_macro, self.cim_cfg.bits_column, 3
        )
        info_tensor = info_tensor[:, 0:macro_num, :, :]
        info_tensor = info_tensor.reshape(
            code["comp_off"], self.cim_cfg.bits_column * macro_num, 3
        )
    ...
    def _run_pim_outsum(self, code):
        ...
        while i < code["out_n"]:
            if code["out_mask"][i] == "1":
                ...
                data1 = self.pim_buffer.read(
                    code["oaddr_off"] + i * self.macro_obw // 8, self.macro_obw // 8
                )
                data2 = self.pim_buffer.read(
                    code["oaddr_off"] + (i + 1) * self.macro_obw // 8,
                    self.macro_obw // 8,
                )
                tensor1 = np.frombuffer(data1, dtype=f"int{self.macro_obw}").reshape(
                    1,
                )
                tensor2 = np.frombuffer(data2, dtype=f"int{self.macro_obw}").reshape(
                    1,
                )
                tensor_new = tensor1 + tensor2
                data_new = tensor_new.tobytes()
                self.pim_buffer.write(
                    code["oaddr_off"] + i * self.macro_obw // 8,
                    self.macro_obw // 8,
                    data_new,
                )
            else:
    ...
    def _run_pim_outsum_move(self, code):
        ...
        for i in range(code["out_n"]):
            data1 = self.pim_buffer.read(
                code["oaddr_off"] + 2 * i * self.macro_obw // 8, self.macro_obw // 8
            )
            data2 = self.pim_buffer.read(
                code["oaddr_off"] + (2 * i + 1) * self.macro_obw // 8,
                self.macro_obw // 8,
            )
            tensor1 = np.frombuffer(data1, dtype=f"int{self.macro_obw}").reshape(
                1,
...
if __name__ == "__main__":
    ...
    with open(os.path.join("save", path, "program.json"), "r") as f:
        ...
        if "type" in layer and layer["type"] == "FCN":
            ...
            out_shape = [b, c, h, w]
    input = (
        np.load(os.path.join("model", path, "input.npy"))
        .reshape(in_shape)
        .astype("int8")
    )
    ...
    output = (
        np.load(os.path.join("model", path, "output.npy"))
        .reshape(out_shape)
        .astype("int8")
    )
    #
    ...
    # Execute
    executor = BitSparseSimulatorExecutor(
        cim_cfg=CimConfig.global_cim_config(), program=program, const=weight_bytes
    )
