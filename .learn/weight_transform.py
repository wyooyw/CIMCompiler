...
def weight_transform(weight, cim_cfg, op_cfg, weight_dtype="OIHW"):
    ...
    new_weight = np.zeros(
        (len(fold), ele_in_filter, cim_cfg.bits_column * cim_cfg.n_macro), dtype="int8"
    )
    info = np.zeros(
        (len(fold), ele_in_filter, cim_cfg.bits_column * cim_cfg.n_macro, 3),
        dtype="int8",
    )
    ...
    new_weight = new_weight.reshape(
        len(fold), ele_in_filter, cim_cfg.bits_column * cim_cfg.n_macro // 8, 8
    )
    info = info.reshape(
        len(fold), ele_in_filter, cim_cfg.bits_column * cim_cfg.n_macro * 3 // 8, 8
    )
