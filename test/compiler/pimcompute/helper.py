...
class TestHelper:
    ...
    def get_output(self, memory_space):
        ...
        output_byte_size = self.out_hw * self.out_hw * self.out_channel * self.output_bytes
        output = memory_space.read_as(output_offset, output_byte_size, self.output_dtype)
...
class ValueSparseConv2dTestHelper(Conv2dTestHelper):
    ...
    def _get_mock_weight(self):
        ...
        weight = np.random.randint(-100,100,size=[self.out_channel, self.ker_size, self.ker_size, self.in_channel],dtype=np.int8)
        mask1 = np.random.randint(0,2,size=[self.out_channel, self.ker_size, self.ker_size, self.in_channel],dtype=np.int8)
        mask2 = np.random.randint(0,2,size=[self.out_channel, self.ker_size, self.ker_size, self.in_channel],dtype=np.int8)
        mask3 = np.random.randint(0,2,size=[self.out_channel, self.ker_size, self.ker_size, self.in_channel],dtype=np.int8)
...
class QuantizeHelper:
    ...
    def get_image_quantify(self, simulator, bias=None, scale=None, out_zp=None, relu=False):
        import numpy as np
