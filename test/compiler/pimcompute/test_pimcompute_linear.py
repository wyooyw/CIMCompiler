...
from utils.predict_pimcompute_count import predict_pimcompute_count_for_linear_dense
...
class TestPIMComputeValueSparse:
    ...
    def test_pim_compute(self, casename, op_config):
        ...
        pimcompute_count = predict_pimcompute_count_for_linear_dense(
        ...
        self.simulator.memory_space.write(image, global_memory_base, len(image))
        status, stats, flat = self.simulator.run_code(
            flat_code, total_pim_compute_count=pimcompute_count
        )
        assert status == self.simulator.FINISH
...
if __name__ == "__main__":
    ...
    tester.setup_method()
    tester.test_pim_compute("linear/bit_sparse", {"out_channel": 32, "in_channel": 16})
