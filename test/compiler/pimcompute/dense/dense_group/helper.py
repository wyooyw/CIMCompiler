class TestHelper:
    ...
    def check_image(self, image):
        ...
        golden_g0 = np.dot(self.input[0,:].astype(np.int32), self.weight[:,0,:].astype(np.int32))
        golden_g1 = np.dot(self.input[1,:].astype(np.int32), self.weight[:,1,:].astype(np.int32))
