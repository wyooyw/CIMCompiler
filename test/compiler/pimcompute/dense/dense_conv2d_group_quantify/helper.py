class TestHelper:
    ...
    def _get_mock_weight(self):
        ...
        weight = np.random.randint(-1, 2, size=(self.out_channel, self.ker_size , self.ker_size, self.in_channel), dtype=np.int8)
        # weight[:,:,:,::2] = 0
        return weight
    ...
    def _get_mock_input(self):
        ...
        input_data = np.random.randint(-2,2,size=(self.in_hw,self.in_hw, self.in_channel), dtype=np.int8)# .reshape(self.in_hw,self.in_hw,1).repeat(self.in_channel, axis=2)
    ...
    def get_image(self, simulator, input=None, weight=None, bias=None, scale=None, out_zp=None, relu=False):
        ...
        assert self.bias.size==self.out_channel, f"{self.bias.size=}, {self.out_channel=}"
        assert self.scale.dtype==np.float32, f"{self.scale.dtype=}"
        assert self.scale.size==self.out_channel, f"{self.scale.size=}, {self.out_channel=}"
    ...
    def get_output(self, memory_space):
        ...
        # i32_output_offset = global_offset + self.output_offset
        # i32_output_byte_size = self.out_hw * self.out_hw * self.out_channel * 4
        # i32_output = memory_space.read_as(i32_output_offset, i32_output_byte_size, np.int32)
        # i32_output = i32_output.reshape(self.out_hw, self.out_hw, self.out_channel)

        i8_output_offset = global_offset + self.output_offset
        i8_output_byte_size = self.out_hw * self.out_hw * self.out_channel
        i8_output = memory_space.read_as(i8_output_offset, i8_output_byte_size, np.int8)
        i8_output = i8_output.reshape(self.out_hw, self.out_hw, self.out_channel)
        return i8_output
    ...
    def check_image(self, memory_space):
        ...
        output_byte_size = self.out_hw * self.out_hw * self.out_channel
        output = memory_space.read_as(output_offset, output_byte_size, np.int8)
