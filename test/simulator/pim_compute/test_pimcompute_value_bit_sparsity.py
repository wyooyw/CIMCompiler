...
class TestSimulatorPIMComputeValueBitSparse:
    ...
    def test_pimcompute_value_bit_sparse_multi_group_threshold2(self):
        ...
        input_buffer_base = self.simulator.memory_space.get_base_of(
            "pim_input_reg_buffer"
        )
        output_buffer_base = self.simulator.memory_space.get_base_of(
        ...
        value_bit_sparse_result = convert_value_bit_sparse_conv2d_weight(
            weight, macro_config
        )
        ...
        self.simulator.memory_space.write(
            value_bit_sparse_weight, weight_addr, value_bit_sparse_weight.size
        )
        # value sparse
        self.simulator.memory_space.write(
            value_sparse_mask, value_sparse_mask_base, value_sparse_mask.size
        )
        # bit sparse
        self.simulator.memory_space.write(
            bis_sparse_meta, bis_sparse_meta_base, bis_sparse_meta.size
        )
        self.simulator.memory_space.write(outsum_mask, outsum_base, outsum_mask.size)
        self.simulator.memory_space.write(
            transfer_mask, transfer_base, transfer_mask.size
        )
        ...
        inst_list = [
            ...
            self.inst_util.special_li(
                SpecialReg.BIT_SPARSE_META_ADDR, bis_sparse_meta_base
            ),
            self.inst_util.special_li(
                SpecialReg.VALUE_SPARSE_MASK_ADDR, value_sparse_mask_base
            ),
            ...
            self.inst_util.pim_output(
                outsum_move=0, outsum=1, out_n=5, output_mask_addr=4, output_addr=3
            ),
            ...
            self.inst_util.pim_transfer(
                src_addr=3, src_size=5, transfer_mask=6, dst_addr=7
            ),
        ...
        for group_id in range(2):
            ...
            output_group = np.dot(
                input_group.astype(np.int32), weight_group.astype(np.int32)
            )
        ...
        output = self.simulator.memory_space.read_as(
            transfer_output_addr, output_size // 2, np.int32
        )
        print(output)
