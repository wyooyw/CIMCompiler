...
class TestSimulatorPIMCompute:
    ...
    def test_pimcompute_dense_single_group_accumulate(self):
        ...
        output_golden = np.dot(
            input.astype(np.int32), weight_A.astype(np.int32)
        ) + np.dot(input.astype(np.int32), weight_B.astype(np.int32))
    ...
    def test_pimcompute_dense_single_group_part(self):
        ...
        weight = np.arange(16, dtype=np.int8).reshape(4, 4)
        output_golden = np.dot(
            input[:3].astype(np.int32), weight[:3, :3].astype(np.int32)
        )
    ...
    def test_pimcompute_dense_multi_group_fix_step(self):
        ...
        inst_list = [
            ...
            self.inst_util.special_li(
                SpecialReg.GROUP_SIZE, self.macro_config.n_macro // group_num
            ),
        ...
        group_weight = np.arange(16, dtype=np.int8).reshape(
            4, 2, 2
        )  # [comp, group, macro_per_group]
        group_output_golden = []
        ...
        self.simulator.memory_space.write(
            group_input, input_addr, input_size * group_num
        )
        ...
        assert (
            output == group_output_golden
        ).all(), f"{output=}, {group_output_golden=}"
    ...
    def test_pimcompute_dense_multi_group_accumulate_fix_step(self):
        ...
        group_input = np.arange(8, dtype=np.int8).reshape(2, 4)
        group_weight = np.arange(32, dtype=np.int8).reshape(
            2, 4, 2, 2
        )  # [row, comp, group, macro_per_group]
        group_output_golden = []
        for group_id in range(group_num):
            input = group_input[group_id]
