...
def conv2d(input, weight, padding, bias=None, stride=1):
    ...
    for row in range(output_hw):
        for col in range(output_hw):
            input = input_pad[:,stride*row:stride*row+ker_size,stride*col:stride*col+ker_size].reshape(-1,1)
...
def check_zeropoint():
    out_wh = 16
    out_channel = 128
    output_feature_path = "models/resnet18/ResNet18_ori_data_0731/7_conv/output_feature.txt"
    ...
    bias = np.loadtxt("models/resnet18/ResNet18_ori_data_0731/7_conv/bias.txt", dtype=np.int32)
    scale = np.loadtxt("models/resnet18/ResNet18_ori_data_0731/7_conv/scale.txt", dtype=np.float32)
    out_zp_data = 0
