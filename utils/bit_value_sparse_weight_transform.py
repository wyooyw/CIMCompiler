...
def convert_value_bit_sparse_conv2d_weight(weight, macro_config):
    ...
    value_bit_sparse_weight = np.repeat(value_bit_sparse_weight, macro_config["n_group"], axis=2)
...
def convert_bit_sparse_conv2d_weight(weight, value_sparse_weight, cim_cfg, mapping_reduce_to_macro, mapping_macro_to_row):
    ...
    for outer_oc in range(0, out_channel, n_filter_per_group):
        ...
        for middle_oc in range(0, n_filter_per_group, n_filter_per_macro):
            ...
            for inner_oc in range(0, n_filter_per_macro):
                ...
                valid = np.logical_or((weight_bit_num[:,:,:,oc]==0), (weight_bit_num[:,:,:,oc]==one_filter_threshold)).all()
        ...
        assert outsum_mask_per_group.shape[0]==n_macro_per_group and outsum_mask_per_group.shape[1]==n_bcol

        transfer_mask_per_group = np.stack(transfer_mask_per_group, axis=0)
        transfer_mask_per_group = np.pad(transfer_mask_per_group, ((0, n_macro_per_group - len(transfer_mask_per_group)), (0,0)), mode='constant', constant_values=0)
    ...
    for i_reduce_to_macro in range(len(mapping_reduce_to_macro)):
        mapping_macro_to_row_end = mapping_macro_to_row_begin + mapping_reduce_to_macro[i_reduce_to_macro]

        for i_macro_to_row in range(mapping_macro_to_row_begin, mapping_macro_to_row_end):
            ...
            for i_row in range(row_begin, row_end):
                ...
                for macro_id in range(n_macro_per_group):
                    ...
                    for filter_id in range(n_filter_per_macro):
                        ...
                        # import pdb; pdb.set_trace()
                        print(f"{value_sparse_weight.shape=}")
                        filter_weight = value_sparse_weight[i_row, :, macro_id, filter_id].reshape(-1) # n_to
                        abs_filter_id = i_reduce_to_macro * n_filter_per_group + macro_id * n_filter_per_macro + filter_id
                        filter_threshold = total_threshold[abs_filter_id]

                        # check threshold
                        assert filter_threshold in [1,2]
                        _filter_threshold = int_to_csd_nonzero_count_tensor(filter_weight)
                        assert np.logical_or((_filter_threshold==0), (_filter_threshold==filter_threshold)).all()
                        ...
                        value,sign,location = parse_tensor(filter_weight, filter_threshold)
    ...
    for ost in range(out_spatial_tile):
        ...
        while reduce_element < total_reduce_size:
            ...
            while row_in_macro < n_row and i_outer_reduce < out_reduce_tile:
                ...
                submask, subweight = extrace_mask_and_data(subweight,  n_from, n_to, concat=True, bit_to_byte=True, strict_align=True) 
