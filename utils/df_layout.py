...
def _int_to_bits(num: int, width: int, endian: str, signed: bool):
    ...
    assert (
        num >= lower_bound and num <= upper_bound
    ), f"When width={width} and signed={signed}, num should in [{lower_bound},{upper_bound}], but got num={num}"

    bits = bitarray(endian=endian)
    bits.frombytes(
        int(num).to_bytes(math.ceil(width / 8), byteorder=endian, signed=signed)
    )
    if endian == "big":
        bits = bits[-width:]
    else:
...
def _bits_to_int(bits_list, endian, signed):
    assert (
        type(bits_list) == list and len(bits_list) % 8 == 0
    ), f"{type(bits_list)},{len(bits_list)}"
    bits = bitarray(bits_list, endian=endian)
    b = bits.tobytes()
...
def _tensor_bits_to_int(tensor, endian: str, signed: True, dtype: type):
    ...
    assert (
        type(endian) == str and endian == "little" or endian == "big"
    ), f"endian={str(endian)}"
    assert type(signed) == bool, f"signed={str(signed)}"
    assert dtype in [
        np.int8,
    ...
    for num in range(length):
        new_tensor[num] = _bits_to_int(
            bits_list=tensor[num, :].tolist(), endian=endian, signed=signed
        )
...
def tensor_bits_to_uint32(tensor, endian="little"):
    ...
    return _tensor_bits_to_int(
        tensor=tensor, endian=endian, signed=False, dtype=np.uint32
    )
...
def tensor_bits_to_int32(tensor, endian="little"):
    ...
    assert tensor.shape[-1] == 32
    return _tensor_bits_to_int(
        tensor=tensor, endian=endian, signed=True, dtype=np.int32
    )
...
def tensor_bits_to_uint16(tensor, endian="little"):
    ...
    return _tensor_bits_to_int(
        tensor=tensor, endian=endian, signed=False, dtype=np.uint16
    )
...
def tensor_bits_to_int16(tensor, endian="little"):
    ...
    assert tensor.shape[-1] == 16
    return _tensor_bits_to_int(
        tensor=tensor, endian=endian, signed=True, dtype=np.int16
    )
...
def tensor_bits_to_uint8(tensor, endian="little"):
    ...
    return _tensor_bits_to_int(
        tensor=tensor, endian=endian, signed=False, dtype=np.uint8
    )
...
def im2col_layout(tensor, kernel_size, padding, stride):
    ...
    format_feature = loop_nest_layout(
        tensor,
        bound=[
            batch,
            (0, height - kernel_size + 1, stride),
            (0, width - kernel_size + 1, stride),
            channel,
            kernel_size,
            kernel_size,
        ],
    ...
    format_feature = format_feature.reshape(
        batch, -1, channel * kernel_size * kernel_size
    )
...
def safely_astype(tensor, dtype):
    ...
    width = {np.int8: 8, np.int32: 32, np.float32: 32}[dtype]
    signed = {np.int8: True, np.int32: True, np.float32: True}[dtype]
    ...
    assert (
        tensor.max() <= upper_bound and tensor.min() >= lower_bound
    ), f"tensor's value should in [{lowerbound,upperboud}], but got tensor.min()={tensor.min()}, tensor.max()={tensor.max()}"
