        ...
        for key, value in stats.items():
            if type(value)==int:
                ...
                for sub_key, sub_value in value.items():
                    ...
                    total_stats[key][sub_key] = total_stats[key].get(sub_key, 0) + sub_value
    return total_stats
...
def stats_model_all_mode(model_path):
    ...
    for mode in ["value_sparse", "bit_sparse", "bit_value_sparse"]:
        ...
        ratio_without_dw[mode] = {
            ...
            "pim": stats_without_dw["pim"]["pim_compute"] / stats_dense_without_dw["pim"]["pim_compute"],
        }
...
def sum_stats_for_macro_utilization(stats_json_list):
    ...
    for stats_json in stats_json_list:
        ...
        assert stats["sum_macro_cell_use"] > 0, f"{stats_json} sum_macro_cell_use={stats['sum_macro_cell_use']}"
        assert stats["sum_macro_cell_total"] > 0, f"{stats_json} sum_macro_cell_total={stats['sum_macro_cell_total']}"
        for key, value in stats.items():
            total_stats[key] = total_stats.get(key, 0) + value
...
def stats_macro_utilization(folder_path):
    ...
    total_stats["use_rate"] = total_stats["sum_macro_cell_use"] / total_stats["sum_macro_cell_total"] if total_stats["sum_macro_cell_total"]>0 else 0

    total_stats_no_dw = sum_stats_for_macro_utilization(stats_json_list_no_dw)
    total_stats_no_dw["use_rate"] = total_stats_no_dw["sum_macro_cell_use"] / total_stats_no_dw["sum_macro_cell_total"] if total_stats_no_dw["sum_macro_cell_total"]>0 else 0
