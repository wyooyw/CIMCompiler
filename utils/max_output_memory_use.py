...
def model_max_out_memory_use(model_config):
    ...
    for layer in model_config["layers"]:
        if layer["type"] == "CONV" or layer["type"] == "FC":
            ...
            print(f"{layer['name']} max out memory use: i32: {layer_use_i32}B, {math.ceil(layer_use_i32/1024)}KB; i8: {layer_use_i8}B, {math.ceil(layer_use_i8/1024)}KB")
            layer_use = layer_use_i32 + layer_use_i8
            print(f"{layer['name']} max out memory use: {layer_use}B, {math.ceil(layer_use/1024)}KB")
