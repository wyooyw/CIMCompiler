...
def get_weight(model_path, layer_info, layer_name):
    ...
    weight = np.loadtxt(weight_path, dtype=np.int8).reshape(layer_info['output_channel'], layer_info["input_channel"], layer_info["weight_row"], layer_info["weight_col"])
...
def filter_zero_ratio(weight):
    ...
    for oc in range(out_channel):
        ...
        if oc % 2 == 1:
            ...
            print(f"    Filter [{oc-1}, {oc}] has common zero count {common_nero_cnt}, ratio {common_zero_ratio}")
...
def twin_filter_zero_ratio(weight):
    ...
    for oc in range(0,out_channel,2):
        ...
        zero_ratio = nero_cnt/weight[oc].size
        print(f"Filter [{oc}, {oc+1}] has common zero count {nero_cnt}, ratio {zero_ratio}")
...
if __name__=="__main__":
    ...
    for j in range(0, weight.shape[1]//128*128, 128):
        ...
        for i in range(16):
            ...
            num_zero = np.logical_and(weight_for_one_macrp[0]==0, weight_for_one_macrp[1]==0).sum()
