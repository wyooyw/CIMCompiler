...
def int_to_csd_nonzero_count_tensor(tensor):
    ...
    if int_to_csd_nonzero_count_loopup_table == None:
        int_to_csd_nonzero_count_loopup_table = (
            torch.zeros((256,)).to(torch.int8).to(tensor.device)
        )
...
def parse_fold_weight(cim_cfg, op_cfg, weight_bit_num):
    ...
    assert (
        type(weight_bit_num) == list and len(weight_bit_num) == op_cfg.out_channel
    ), f"{len(weight_bit_num)}, {op_cfg.out_channel}"
    whole = []
    channel_idx = 0
    while channel_idx < op_cfg.out_channel:
        ...
        bit_idx = 0
        while (channel_idx < op_cfg.out_channel) and (
            bit_idx + weight_bit_num[channel_idx] <= max_bit_width
        ):
...
def weight_transform(weight, cim_cfg, op_cfg, weight_dtype="OIHW"):
    ...
    weight_bit_num_per_out_channel = int_to_csd_nonzero_count_tensor(
        weight[0:1, 0:1, 0:1, :]
    )
    # ipdb.set_trace()
    assert (
        weight_bit_num == weight_bit_num_per_out_channel
    ).all(), "Element in one filter should have same threshold."

    fold = parse_fold_weight(cim_cfg, op_cfg, weight_bit_num_per_out_channel)

    padded_ele_in_filter = (
        int(math.ceil(ele_in_filter / cim_cfg.n_comp)) * cim_cfg.n_comp
    )
    ...
    new_weight = np.zeros(
        (len(fold), padded_ele_in_filter, cim_cfg.bits_column * cim_cfg.n_macro),
        dtype="int8",
    )
    info = np.zeros(
        (len(fold), padded_ele_in_filter, cim_cfg.bits_column * cim_cfg.n_macro, 3),
        dtype="int8",
    )
    ...
    new_weight = new_weight.reshape(
        len(fold), padded_ele_in_filter, cim_cfg.bits_column * cim_cfg.n_macro // 8, 8
    )
    info = info.reshape(
        len(fold),
        padded_ele_in_filter,
        cim_cfg.bits_column * cim_cfg.n_macro * 3 // 8,
        8,
    )
...
def parse_out_mask_and_transfer_mask(fold, padding_to):
    ...
    for i in range(len(fold)):
        ...
        out_mask = np.pad(
            out_mask,
            ((0, padding_to - out_mask.shape[0])),
            mode="constant",
            constant_values=0,
        )
        transfer_mask = np.pad(
            transfer_mask,
            ((0, padding_to - transfer_mask.shape[0])),
            mode="constant",
            constant_values=0,
        )
        ...
        pimset_mask = np.pad(
            pimset_mask,
            ((0, padding_to - pimset_mask.shape[0])),
            mode="constant",
            constant_values=0,
        )
...
if __name__ == "__main__":
    ...
    value, sign, location = parse_tensor(
        weight[0].reshape(-1), int_to_csd_nonzero_count(weight[0, 0, 0, 0])
    )
    # print("value:")
    # print(value)
    # print("sign:")
