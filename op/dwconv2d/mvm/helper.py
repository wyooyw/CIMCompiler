...
class TestHelper(DenseConv2dTestHelper, QuantizeHelper):
    ...
    def _make_dense_data(self, weight, simulator):
        ...
        converted_weight, pimset_mask = convert_dense_depthwise_conv2d_weight(
            weight, config
        )

        assert len(converted_weight.shape) == 4, f"{converted_weight.shape=}"
        assert (
            converted_weight.shape[0] == self.out_channel
        ), f"{converted_weight.shape=}, {self.out_channel=}"
        assert (
            converted_weight.shape[1] == self.ker_size * self.ker_size
        ), f"{converted_weight.shape=}, {self.ker_size=}"
        assert (
            converted_weight.shape[2] == n_group
        ), f"{converted_weight.shape=}, {n_group=}"
        assert (
            converted_weight.shape[3] == n_group_vcol
